...
async function verifyUser(ctx, next) {
    ...
    let isMember = memberCache.get(userId);

    // لو مش متأكدين إنه عضو → اسأل تيليجرام
    if (isMember !== true) {
        isMember = await checkMembership(userId);

        // لو مش عضو خزّن النتيجة 3 ثواني بس
        if (isMember === false) {
            memberCache.set(userId, false);
            setTimeout(() => memberCache.delete(userId), 3000);
        }
    }
...
async function startXeony() {
    bot.on('callback_query', async (XeonBotInc) => {
        try {
            ...
            await XeonBotInc.answerCbQuery().catch(() => {});
            ...
            const reply = async (text) => {
                for (let x of simple.range(0, text.length, 4096)) {
                    await XeonBotInc.replyWithMarkdown(
                        text.substr(x, 4096),
    ...
    try {
        ...
        if (profilePhotos.photos.length > 0) {
            ...
            await XeonBotInc.replyWithPhoto(ownerPhoto, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'OWNER', url: "https://t.me/med_tar" }, { text: 'CHANNEL', url: "https://t.me/b_o_d_a1FS" }, { text: 'GROUP', url: "https://t.me/b_o_da12" }]
                    ]
                }
            });
        } else {
        ...
        await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: 'OWNER', url: "https://t.me/med_tar" }, { text: 'GROUP', url: "https://t.me/b_o_da12" }]
                ]
            }
        });
    }
...
*Hold on it to copy the ID.*`;
    ...
    bot.on('message', verifyUser, async (XeonBotInc) => {
...
const sessionPath = `./session/${global.currentPhone}`
...
// 🔁 Auto reconnect (مهم)
XeonBotInc.ev.removeAllListeners('connection.update')
...
let code = await XeonBotInc.requestPairingCode(phoneNumber, 'ZEUSBODA');
code = code?.match(/.{1,4}/g)?.join("-") || code;
console.log(`Code :`, code);
}

// 🔥 امنع التكرار (مهم جدًا)
XeonBotInc.ev.removeAllListeners('messages.upsert');
...
(async () => {
    try {
        // 👈 أضف السطر ده
        global.currentPhone = "22242203253"; // رقمك بدون +
...
// ================= KEEP ALIVE =================
setInterval(() => {
    console.log("🟢 Bot still alive:", new Date().toLocaleString());
}, 30000);
// =============================================
