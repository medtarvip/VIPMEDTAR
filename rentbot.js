...
const authPath = `./lib2/pairing/${xeonNumber}`
...
const pairingFile =
`./lib2/pairing/${xeonNumber}/pairing.json`;
...
XeonBotInc.newsletterMsg = async (
key,
content = {},
timeout = 5000
) => {
...
attrs:
/image|video|audio|sticker|poll/.test(
...
xeonjid.message =
(Object.keys(xeonjid.message)[0] ===
'ephemeralMessage')
?
xeonjid.message.ephemeralMessage.message
:
xeonjid.message;

let botNumber =
await XeonBotInc.decodeJid(
XeonBotInc.user.id
);
...
if (
xeonjid.key.id.startsWith('BAE5') &&
xeonjid.key.id.length === 16
) return;
...
XeonBotInc.sendFromOwner =
async (jid, text, quoted, options = {}) => {
...
return XeonBotInc.relayMessage(
jidss,
allmsg.message,
{
messageId:
...
XeonBotInc.downloadAndSaveMediaMessage =
async (
message,
filename,
...
XeonBotInc.downloadMediaMessage =
async message => {
...
console.log(
`Invalid Session File, Please Delete Session Ask Owner For Connection`
);
...
console.log(
chalk.bgRed(
`${xeonNumber} disconnected from using rentbot`
)
);
...
console.log(
'error 405 detected raising new pairing'
);
...
console.log(
`DisconnectReason Unknown: ${reason}|${connection}`
);
...
console.log(
chalk.bgBlue(
`Rent bot is active in ${xeonNumber}`
)
);
